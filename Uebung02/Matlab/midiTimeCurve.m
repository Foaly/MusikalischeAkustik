function newtime=midiTimeCurve(fname,midi,info,ext,timecrv,endtime);
% function newtime=midiwr(fname,midi,info,ext,timecrv,endtime)
%
...
%
%  This is the Mini-Mazzola (mm) version of midiwr... 
%
%      timecrv      gives the velocity w.r.t. original performance, this
%                   vector is scaled proportinally to the full length of the midi-piece.
%                   Values > 1 mean faster, < 1 slower speed than the original.
%
%      endtime      only write notes until endtime is reached
%
%      newtime      new (interpolated) time curve
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%   Last changes by Frank Kurth, May, 10th, 2004
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%----------------------------
%% begin changes w.r.t. midiwr
if (nargin < 6)
   endtime = Inf;
end
if (nargin < 5) | isempty(timecrv)
   timecrv = [1];
end

ltime = length(timecrv);
absduration = max([ext.abstime]);       % length estimation of MIDI file in delta-ticks
for j = 1:info.ntrks                    % calculated from sideinfo and note-info
   if max(midi{j}(:,2)) > absduration
      absduration = max(midi{j}(:,2));
   end
end
...
timeGrid = (0:ltime-1);
timeGrid = timeGrid*(absduration/(ltime-1));
scalefactor = INTERP1(timeGrid,timecrv,0:absduration,'linear');

newtime = zeros(1,absduration+1);
for j=2:absduration+1
    newtime(j)=newtime(j-1)+scalefactor(j);    
end

%% end changes w.r.t. midiwr
%----------------------------
...
for k = 1:info.ntrks
   ...
   while (extptr <= length(curextidx)) | (noteptr<=lx)
     ...
     if( curexttime(extptr) <= noteevents(noteptr,2) )   
         ...
         %----------------------------
         %% begin changes w.r.t. midiwr      
         if curexttime(extptr) ~= inf
             
            delta = round(newtime(1+curexttime(extptr))-abstime);
            abstime = newtime(1+curexttime(extptr));

         else

            delta = round(newtime(1+curexttime(extptr-1))-abstime);
            abstime = inf; %curexttime(extptr)*scalefactor(1+curexttime(extptr-1));
         ...
         %----------------------------
         %% begin changes w.r.t. midiwr
         delta = round(newtime(1+(noteevents(noteptr,2)))-abstime);
              
         abstime = newtime(1+noteevents(noteptr,2));
         delta = WriteVarLenIntern(delta);
         %% end changes w.r.t. midiwr
         %----------------------------
